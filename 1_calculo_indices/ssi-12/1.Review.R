# ============================================================================
# SSI-12 calculation — Sao Francisco basin
# Scenarios: Historical (1985-2014), SSP126 (2041-2070), SSP585 (2041-2070)
# Baseline: FIXED historical 1965-2014 (50 yrs) for ALL scenarios
...
path_flo_base <- "/data/brussel/vo/000/bvo00033/vsc11346/SSI_R/2.Codigos_SSI/3.Outputs_flo_out/Sao_Francisco/"
path_gap_base <- "/data/brussel/vo/000/bvo00033/vsc11346/SSI_R/0.Gap_filling_2015-2020/2.Outputs_gap_filling/Sao_Francisco/"
path_out_base <- "/data/brussel/vo/000/bvo00033/vsc11346/SSI_R/3.Outputs_SSI/3.baseline_fixed_hist/Sao_Francisco/"

# ============================================================================
# SCENARIO CONFIGURATION
# ============================================================================
# NOTE: ref.start/ref.end now fixed to 1965-2014 for ALL scenarios, so that
# the Gamma distribution is fitted ONLY on the historical period, and the
# same reference climate is used to evaluate historical, ssp126, and ssp585.
# This makes SSI comparable across scenarios.
# ============================================================================
...
scenarios <- list(
    ...
    ssp126 = list(
        ...
        time_ref_start = c(1965, 1),
        time_ref_end   = c(2014, 12),   # <-- FIXED: historical baseline only
        n_baseline     = 1272,
        n_analysis     = 360,
        skip_months    = 912,
        is_future      = TRUE
    ...
    for (model in models) {
        ...
        ssi_var  <- ncvar_def(
            ...
            longname = paste0(
                ...
                "baseline FIXED 1965-2014 for all scenarios, ",
                "mask Strahler s5)"
            )
        )
...
print("Done — Sao Francisco historical + SSP126 + SSP585 SSI-12 (fixed baseline 1965-2014).")